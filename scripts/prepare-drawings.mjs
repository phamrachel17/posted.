...
const OUTPUTS = [
  ...
  { src: "dancing_couple_outline.PNG", name: "logo", shape: "natural", size: 900, thicken: 3 },
  { src: "dancing_couple_outline.PNG", name: "logo-small", shape: "natural", size: 240, thicken: 10 },
  // In full color (not an ink mask): the couple on the record player.
  { src: "dancing_couple_complete.PNG", name: "couple-color", shape: "natural", size: 600, thicken: 0, color: true },
...
async function prepare({ src, name, shape, size, thicken, boost = 4, fill = false, color = false }) {
  const file = `drawings/${src}`;
  const box = await inkBox(file);
  const pad = Math.round(Math.max(box.width, box.height) * PAD) + thicken;

  // A color drawing keeps its own colors: just crop, pad, and resize it.
  if (color) {
    const pad0 = Math.round(Math.max(box.width, box.height) * PAD);
    const scale0 = size / Math.max(box.width + pad0 * 2, box.height + pad0 * 2);
    const padded = await sharp(file)
      .ensureAlpha()
      .extract(box)
      .extend({ top: pad0, bottom: pad0, left: pad0, right: pad0, background: { r: 0, g: 0, b: 0, alpha: 0 } })
      .png()
      .toBuffer();
    const out = await sharp(padded)
      .resize(Math.round((box.width + pad0 * 2) * scale0), Math.round((box.height + pad0 * 2) * scale0), { kernel: "lanczos3" })
      .png({ compressionLevel: 9 })
      .toFile(`public/doodles/${name}.png`);
    console.log(`${name}.png`.padEnd(22), `${out.width}×${out.height}`, "(color)");
    return;
  }
...
const names = [...new Set(OUTPUTS.filter((o) => !o.color).map((o) => o.name))].sort();
writeFileSync(
  "src/lib/drawn.ts",
